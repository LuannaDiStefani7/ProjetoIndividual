...
function listarPorUsuario(idUsuario) {
    ...
    var instrucao = `
    SELECT idResenha, titulo, assunto,resenha, fkUsuario, exibir,idUsuario, nome, email,senha
        FROM Resenha  INNER JOIN usuario  ON fkUsuario  = idUsuario WHERE idUsuario = '${idUsuario}';
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function deletar(idResenha) {
    console.log("ACESSEI O AVISO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function deletar():", idResenha);
    var instrucao = `
    update resenha set exibir = 'f' where idResenha = ${idResenha};
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
